...
- Real file/photo upload to cloud storage — the custody-verification endpoints accept a storage *reference* (string) as if a signed-upload flow already happened; there's no real upload flow since that needs a Firebase/GCS project
- Firebase Auth — this codebase uses local JWT auth instead (see the note below)
- SMS/masked calling (Twilio)
- Real Stripe payment capture — the payment *lifecycle* (authorize/capture/refund, ledger, driver payouts) is now real and tested against a `MockPaymentProvider`; swapping in actual Stripe just means implementing the same `PaymentProvider` interface with real API keys (see `payment.provider.ts`) — nothing else in the codebase needs to change
